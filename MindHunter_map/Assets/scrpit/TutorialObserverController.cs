...
public class TutorialObserverController : MonoBehaviour
{
    ...
    {
        alertSecond = 0f;
        mapConfig = new MapConfig();//GameObject.FindObjectOfType<AutoGenerateMap>().config;
    ...
    void unAlert()
    {
        foreach (var fighter in fighters)
        {
            ...
            fighter.GetComponent<TutorialOtherController>().unAlert();
        }
        musicController.Alert.Stop();
        Debug.Log("unalert");
    }
    public void alertFighter(Vector2Int? playerRoom)
    {
        ...
        {
            if (fighter == null) continue;
            fighter.GetComponent<TutorialOtherController>().alert(playerRoom.Value);
        }
        Debug.Log("alert");
        alertSecond = 30;
